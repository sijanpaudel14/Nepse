...
_SCAN_POOL = ThreadPoolExecutor(max_workers=30, thread_name_prefix="scan")

# Background job store for long-running scans (Stealth full scan)
_stealth_jobs: Dict[str, Dict[str, Any]] = {}
_stealth_executor = ThreadPoolExecutor(max_workers=30)  # one full scan at a time
    ...
    try:
        ...
        def _do_scan():
            ...
            results = screener.run_full_analysis(
                quick_mode=quick, max_workers=30, cancel_event=cancel_event
            )
    ...
    try:
        ...
        if quick:
            ...
            def _do_quick_stealth():
                screener = MasterStockScreener(strategy="momentum", target_sector=sector, max_price=max_price)
                results = screener.run_full_analysis(
                    quick_mode=True, max_workers=30, cancel_event=cancel_event
                )
